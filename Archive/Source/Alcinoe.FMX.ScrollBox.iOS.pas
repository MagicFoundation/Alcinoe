...
begin
  var LContentOffset: NSPoint := View.contentOffset;
  LContentOffset.x := LContentOffset.x + X;
  LContentOffset.y := LContentOffset.y + Y;
  View.setContentOffset(LContentOffset, aAnimated);
end;
...
begin
  ...
  if (Form <> nil) and IsOurPanGesture then begin
    var LCaptured := Form.Hovered;
    var LCapturedControl: IGestureControl;
    if Supports(LCaptured, IGestureControl, LCapturedControl) then Result := LCapturedControl.GetFirstControlWithGesture(TInteractiveGesture.Pan) <> nil
    else Result := False;
  end
...
begin
  ...
  if Form <> nil then begin
    ...
    var LTouchPoint := gestureRecognizer.locationInView(WindowHandleToPlatform(Form.Handle).View);
    case gestureRecognizer.state of

      UIGestureRecognizerStateBegan: FPanGestureRecognized := True;

      UIGestureRecognizerStateChanged: Form.MouseMove([ssLeft, ssTouch], LTouchPoint.x, LTouchPoint.y);

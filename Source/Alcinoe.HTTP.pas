...
type
  ...
  {------------------------------------}
  TALHTTPRequestHeaders = Class(TObject)
  protected
  ...
  protected
    function PropertyIndexToName(const AIndex: Integer): AnsiString; virtual;
    function NameToPropertyIndex(const AName: AnsiString): Integer; virtual;
    function GetCookies: TObjectList<TALHTTPCookie>; virtual; abstract;
    function GetUnknownHeaders: TALStringsA; virtual; abstract;
    function GetHeaderValueByPropertyIndex(const Index: Integer): AnsiString; virtual; abstract;
    procedure SetHeaderValueByPropertyIndex(const Index: Integer; const Value: AnsiString); virtual; abstract;
    function GetHeaderValueByName(const AName: AnsiString): AnsiString; virtual;
  ...
  public
    ...
    property Cookies: TObjectList<TALHTTPCookie> read GetCookies;
    property UnknownHeaders: TALStringsA read GetUnknownHeaders;
    property Values[const AName: AnsiString]: AnsiString read GetHeaderValueByName write SetHeaderValueByName; default;
    property RawHeaderText: AnsiString read GetRawHeaderText write setRawHeaderText;
  end;
  ...
  protected
    ...
    function GetHeaders: TALHTTPRequestHeaders; virtual; abstract;
  ...
  public
    ...
    property Headers: TALHTTPRequestHeaders read GetHeaders;
  ...
  protected
    ...
    function GetHeaders: TALHTTPResponseHeaders; virtual; abstract;
    function GetBodyStream: TStream; virtual; abstract;
    procedure SetBodyStream(const AValue: TStream); virtual; abstract;
    function GetOwnsBodyStream: Boolean; virtual; abstract;
    procedure SetOwnsBodyStream(const AValue: Boolean); virtual; abstract;
  ...
  public
    ...
    property Headers: TALHTTPResponseHeaders read GetHeaders;
    property BodyStream: TStream read GetBodyStream write SetBodyStream;
    property BodyString: AnsiString read GetBodyString write SetBodyString;
    property OwnsBodyStream: Boolean read GetOwnsBodyStream write SetOwnsBodyStream;
  end;
...
{*******************************}
constructor TALHTTPCookie.Create;
begin
  inherited Create;
  Reset;
end;
...
begin
  ...
  try
    ...
    if Expires <> ALNullDate then begin
      ...
      SB.Append('; Expires=');
      SB.Append(AlRfc822DayOfWeekNames[DayOfWeek(Expires)]);
      SB.Append(', ');
      if LDay <= 9 then SB.Append('0'); SB.Append(ALIntToStrA(LDay));
      SB.Append(' ');
      SB.Append(ALRfc822MonthOfTheYearNames[LMonth]);
      SB.Append(' ');
...
{************************************************************************************}
function TALHttpRequestHeaders.PropertyIndexToName(const AIndex: Integer): AnsiString;
begin
  Case AIndex of
    0: Result := 'Accept';
...
{***********************************************************************************}
function TALHttpRequestHeaders.NameToPropertyIndex(const AName: AnsiString): Integer;
begin
...
{******************************************************************************************************}
procedure TALHttpRequestHeaders.SetHeaderValueByName(const AName: AnsiString; const AValue: AnsiString);
begin
  Var LIndex := NameToPropertyIndex(AName);
  If LIndex >= 0 then
...
{*************************************************************************************}
function TALHttpResponseHeaders.PropertyIndexToName(const AIndex: Integer): AnsiString;
begin
  Case AIndex of
...
{************************************************************************************}
function TALHttpResponseHeaders.NameToPropertyIndex(const AName: AnsiString): Integer;
begin
  var LLowerName := ALLowerCase(AName);
...
{****************************************************************************************}
function TALHttpResponseHeaders.GetHeaderValueByName(const AName: AnsiString): AnsiString;
begin
  Var LIndex := NameToPropertyIndex(AName);
  If LIndex >= 0 then Result := GetHeaderValueByPropertyIndex(LIndex)
...
Begin
  ...
  Try
    ...
    var LMonthLabel := LDateTimeLst[1]; // Mon
                                        ...
                                        // Aug
    var LMonth: Word := 1;
    While (LMonth <= 12) and (not ALSameTextA(ALRfc822MonthOfTheYearNames[LMonth],LMonthLabel)) do inc(LMonth);

...
type
  ...
  private
    ...
    FPath: String;
    FQueryString: String;
    FQueryParams: TALNVStringListW;
    FAnchor: String;
    function GetQueryString: String;
    procedure SetQueryString(const AValue: String);
    function GetQueryParams: TALStringsW;
    procedure QueryParamsChanged(Sender: TObject);
  public
    constructor Create(const AUrl: String; Const ADecode: Boolean = False; Const APlusAsSpaces: Boolean = True); virtual;
    destructor Destroy; override;
    procedure Clear; virtual;
    procedure CrackUrl(const AUrl: String; Const ADecode: Boolean = False; Const APlusAsSpaces: Boolean = True); virtual;
    function GetFullUrl(const AEncode: Boolean = False; Const ASpacesAsPlus: Boolean = True): String;
    ...
    property QueryParams: TALStringsW read GetQueryParams;
...
{*******************************}
destructor TALCookedUrlW.Destroy;
begin
  ALFreeAndNil(FQueryParams);
  inherited;
end;
...
begin
  ...
  {$REGION 'Scheme'}
  P1 := ALPosW('://', AUrl);  // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                              //    ^P1
  If P1 > 0 then begin
    FScheme := AlCopyStr(AUrl, 1, P1-1); // ftp
    P1 := P1 + 3; // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
  ...
  var LPathPos := ALPosW('/',AUrl); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                    //     ^ ^P1
                                    //     ^LPathPos
  if LPathPos = P1 - 2 then LPathPos := ALPosW('/',AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                                              //                                    ^LPathPos
  var LQueryPos := ALPosW('?',AUrl); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                     //       ^P1                          ^LPathPos         ^LQueryPos
  var LAnchorPos := ALPosW('#',AUrl);  // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                       //       ^P1                          ^LPathPos         ^LQueryPos    ^LAnchorPos
  if (LPathPos <= 0) then LPathPos := length(AUrl)+1;
  if (LQueryPos <= 0) then LQueryPos := length(AUrl)+1;
  if (LAnchorPos <= 0) then LAnchorPos := length(AUrl)+1;
  ...
  {$REGION 'UserName/Password'}
  if FScheme <> '' then begin
    P3 := ALPosW('@',AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                //       ^P1       ^P3                ^P2
    If (P3 > 0) and (P3 < P2) then begin
      P4 := ALPosW(':', AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                   //       ^P1 ^P4   ^P3                ^P2
      if (P4 > 0) and (P4 < P3) then begin
        FUserName := ALcopyStr(AUrl, P1, P4-P1);
        inc(P4); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
  ...
  if FScheme <> '' then begin
    P3 := ALPosW(']',AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
                                //                  ^P1         ^P3   ^P2
    if (P3 > 0) and (P3 < P2) then P4 := ALPosW(':', AUrl, P3+1) // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
    else P4 := ALPosW(':', AUrl, P1);                            //                  ^P1          ^P4  ^P2
    if (P4 > 0) and (P4 < P2) then begin
      FHost := ALcopyStr(AUrl, P1, P4-P1);
      inc(P4); // ftp://xxxx:yyyyy@[2001:db8::1]:8443/path/filename.xxx?param1=value1#anchor
               //                  ^P1           ^P4 ^P2
      ...
      FHost := ALcopyStr(AUrl, P1, P2-P1);
      if ALSameTextW(FScheme, 'http') then FPort := 80
      else if ALSameTextW(FScheme, 'https') then FPort := 443
      else if ALSameTextW(FScheme, 'ws') then FPort := 80
      else if ALSameTextW(FScheme, 'wss') then FPort := 443
      else if ALSameTextW(FScheme, 'ftp') then FPort := 21
    end;
    P1 := P2; // ftp://xxxx:yyyyy@[2001:db8::1]:21/path/filename.xxx?param1=value1#anchor
              //                                  ^P1
  end;
  ...
  {$REGION 'Anchor'}
  P2 := ALPosW('#',AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:21/path/filename.xxx?param1=value1#anchor
                              //                                  ^P1                             ^P2
  if P2 > 0 then FAnchor := ALCopyStr(AUrl, P2+1, MaxInt)
  else P2 := Length(AUrl) + 1;
  {$ENDREGION}
  ...
  {$REGION 'QueryString'}
  P3 := ALPosW('?',AUrl, P1); // ftp://xxxx:yyyyy@[2001:db8::1]:21/path/filename.xxx?param1=value1#anchor
                              //                                  ^P1               ^P3           ^P2
  if (P3 > 0) and (P3 < P2) then FQueryString := ALCopyStr(AUrl, P3+1, P2-P3-1)
  else P3 := P2;
  {$ENDREGION}
...
begin
  Var LLength := length(FScheme) + 3{://} +
                 ...
                 length(FAnchor) + 1{#};
  if AEncode then LLength := LLength * 3;
  var SB := TALStringBuilderW.Create(LLength);
  try
    ...
    if AEncode then begin
      ...
      var LPort := FPort;
      if ALSameTextW(FScheme, 'http') then begin
        if LPort = 80 then LPort := 0;
      end
      else if ALSameTextW(FScheme, 'https') then begin
        if LPort = 443 then LPort := 0;
      end
      else if ALSameTextW(FScheme, 'ws') then begin
        if LPort = 80 then LPort := 0;
      end
      else if ALSameTextW(FScheme, 'wss') then begin
        if LPort = 443 then LPort := 0;
      end
      else if ALSameTextW(FScheme, 'ftp') then begin
        if LPort = 21 then LPort := 0;
      end;
      if LPort <> 0 then begin
        SB.Append(':');
        SB.Append(ALIntToStrW(LPort));
      end;
      SB.Append(ALPercentEncode(FPath, ['A'..'Z','a'..'z','0'..'9','-','.','_','~', '/'], false{ASpacesAsPlus}));
      if QueryParams.Count <> 0 then begin
        SB.Append('?');
...
{*************************************************}
function TALCookedUrlW.GetQueryParams: TALStringsW;
begin
  If FQueryParams = nil then begin
    FQueryParams := TALNVStringListW.Create;
    FQueryParams.LineBreak := '&';
    FQueryParams.TrailingLineBreak := False;
    FQueryParams.Text := FQueryString;
    FQueryParams.OnChanged := QueryParamsChanged;
...
{**********************************************************}
procedure TALCookedUrlW.QueryParamsChanged(Sender: TObject);
begin
  FQueryString := '';
end;

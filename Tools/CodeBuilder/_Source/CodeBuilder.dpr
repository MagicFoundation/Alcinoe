...
Procedure BuildAlcinoeFMXDynamicControls;
  ...
  begin
    ...
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseEnter(const AObject: TControl);','ChildrenMouseEnter(const AObject: TALDynamicControl);');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseLeave(const AObject: TControl);','ChildrenMouseLeave(const AObject: TALDynamicControl);');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseDown(const AObject: TALDynamicControl; Button: TMouseButton; Shift: TShiftState; X, Y: Single); virtual;','ChildrenMouseDown(const AObject: TALDynamicControl; Button: TMouseButton; Shift: TShiftState; X, Y: Single); override;');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseMove(const AObject: TALDynamicControl; Shift: TShiftState; X, Y: Single); virtual;','ChildrenMouseMove(const AObject: TALDynamicControl; Shift: TShiftState; X, Y: Single); override;');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseUp(const AObject: TALDynamicControl; Button: TMouseButton; Shift: TShiftState; X, Y: Single); virtual;','ChildrenMouseUp(const AObject: TALDynamicControl; Button: TMouseButton; Shift: TShiftState; X, Y: Single); override;');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseEnter(const AObject: TALDynamicControl); virtual;','ChildrenMouseEnter(const AObject: TALDynamicControl); override;');
    aSrc := FindAndReplace(aSrc, 'ChildrenMouseLeave(const AObject: TALDynamicControl); virtual;','ChildrenMouseLeave(const AObject: TALDynamicControl); override;');
...
begin
  ...
  NormalizeSourceCode(LOutputImplementation);
  //-----
  ReplaceAutoGeneratedRegion(LAlcinoeFMXStylesPas{ASource}, 2{ARegionNumber}, ALTrim(LOutputImplementation){AReplacementText});
  //-----
  var LOutputInterface: AnsiString := '';
  var P1 := AlposA(#13#10'implementation'#13#10, LAlcinoeFMXStylesPas);
  if P1 <= 0 then raise Exception.Create('Error B2AE0BE0-B8C9-4BDB-9755-6A61BD3C42C2');
  var P2 := ALposA('procedure ALReset', LAlcinoeFMXStylesPas, P1);
  P1 := ALposA('procedure ALApply', LAlcinoeFMXStylesPas, P1);
  While (P1 > 0) or (P2 > 0) do begin
    if (P1 > 0) and (P2 > 0) then P1 := Min(P1, P2)
    else if (P2 > 0) then P1 := P2;
    P2 := AlposA(#13, LAlcinoeFMXStylesPas, P1);
    if P2 <= 0 then raise Exception.Create('Error 7B5D5C0D-81CB-4E73-80EC-F5D1E3EB0713');
    LOutputInterface := LOutputInterface + ALCopyStr(LAlcinoeFMXStylesPas, P1, P2-P1) + #13#10;
    P2 := ALposA('procedure ALReset', LAlcinoeFMXStylesPas, P1+1);
    P1 := ALposA('procedure ALApply', LAlcinoeFMXStylesPas, P1+1);
  end;
  ReplaceAutoGeneratedRegion(LAlcinoeFMXStylesPas{ASource}, 1{ARegionNumber}, ALTrim(LOutputInterface){AReplacementText});
  //-----

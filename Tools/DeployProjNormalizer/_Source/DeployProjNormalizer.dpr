...
{*********************************************************************************************}
procedure _NormalizeDeployProjFile(Const aDProjFilename: String; Const ACreateBackup: Boolean);
begin
  var LProjectName := ALExtractFileName(AnsiString(ADProjFilename), true{RemoveFileExt});
  var LDeployProjFilename := ALExtractFilePath(ADProjFilename) + String(LProjectName) + '.deployproj';
  ...
  Try
    ...
    LDProjXmlDoc.LoadFromFile(ADProjFilename);
    ...
    for var I := 0 to LDProjXmlDoc.DocumentElement.ChildNodes.Count - 1 do begin
      ...
      if (LPropertyGroupNode.NodeName = 'PropertyGroup') and
        ...
        if ALSameTextA(LFrameworkTypeNode.Text, 'VCL') or
           ALSameTextA(LFrameworkTypeNode.Text, 'NONE') then Exit
        else if not ALSameTextA(LFrameworkTypeNode.Text, 'FMX') then raise Exception.Createfmt('Unsupported Framework (%s)!', [LFrameworkTypeNode.Text]);
      end;
    end;
    ...
    for var I := 0 to LplatForms.Count - 1 do begin
      ...
      for var J := 0 to LConfigs.Count - 1 do begin
        ...
        //OSXARM64
        //OSX64_OutputDir
        if ALSameTextA(LplatFormName, 'OSXARM64') and ALSameTextA(LConfigName, 'Release') then begin
...
begin
  ...
  try
    ...
    If Tfile.Exists(LDProjFilename) then _NormalizeDeployProjFile(LDProjFilename, LCreateBackup)
    else if TDirectory.Exists(LRootDirectory) then begin
      var LDprojFiles := TDirectory.GetFiles(LRootDirectory, '*.dproj', TSearchOption.soAllDirectories);
      for var LDprojFile in LDprojFiles do
        _NormalizeDeployProjFile(LDprojFile, LCreateBackup)
    end
    else raise Exception.Create('Usage: DeployProjNormalizer.exe -DProj="<DprojFilename>" -CreateBackup=<true/false>');

...
type
  ...
  protected class threadvar
    FBackGroundContext: EAGLContext;
  protected
...
begin
  if TThread.CurrentThread.ThreadID = MainThreadID then begin
    if FSharedContext = nil then begin
    ...
    if FBackGroundContext = nil then begin
      FBackGroundContext := TEAGLContext.Wrap(TEAGLContext.Alloc.initWithAPI(FSharedContext.API, FSharedContext.sharegroup));
      TEAGLContext.OCClass.setCurrentContext(FBackGroundContext);
    end;
  end;
end;
...
begin
  ...
  begin
    DestroyPrograms;
    FSharedContext := nil;
    //this procedure is call only in UnregisterContextClasses,
    //so we don't need to take care about all the FBackGroundContext
  end;
end;

class function TCustomContextIOS.IsContextAvailable(const aContextID: integer = 0): Boolean; // https://quality.embarcadero.com/browse/RSP-19160
begin
  ...
  if TThread.CurrentThread.ThreadID = MainThreadID then begin
    ...
    Result := FBackGroundContext <> nil;
  end;
end;

// https://quality.embarcadero.com/browse/RSP-19160
class function TCustomContextIOS.getContextID: Integer;
begin
...
initialization
  TCustomContextIOS.fContextID := 0; // https://quality.embarcadero.com/browse/RSP-19160

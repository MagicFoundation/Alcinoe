...
{ Texture }
  ...
  public
    ...
    property ContextID: integer read FContextID; // https://quality.embarcadero.com/browse/RSP-19160
  end;
...
{ Lights }
  ...
  const
    ltDirectional = TLightType.Directional deprecated 'Use TLightType.Directional';
    ltPoint = TLightType.Point deprecated 'Use TLightType.Point';
    ltSpot = TLightType.Spot deprecated 'Use TLightType.Spot';
  end;
...
{ Context }
  ...
  const
    pjCamera = TProjection.Camera deprecated 'Use TProjection.Camera';
    pjScreen = TProjection.Screen deprecated 'Use TProjection.Screen';
  end;

  TMultisample = (None, TwoSamples, FourSamples);

  TMultisampleHelper = record helper for TMultisample
  const
    msNone = TMultisample.None deprecated 'Use TMultisample.None';
    ms2Samples = TMultisample.TwoSamples deprecated 'Use TMultisample.TwoSamples';
    ms4Samples = TMultisample.FourSamples deprecated 'Use TMultisample.FourSamples';
  end;
  ...
  const
    ctColor = TClearTarget.Color deprecated 'Use TClearTarget.Color';
    ctDepth = TClearTarget.Depth deprecated 'Use TClearTarget.Depth';
    ctStencil = TClearTarget.Stencil deprecated 'Use TClearTarget.Stencil';
  end;
  ...
  TStencilOpHelper = record helper for TStencilOp
  const
    soKeep = TStencilOp.Keep deprecated 'Use TStencilOp.Keep';
    soZero = TStencilOp.Zero deprecated 'Use TStencilOp.Zero';
    soReplace = TStencilOp.Replace deprecated 'Use TStencilOp.Replace';
    soIncrease = TStencilOp.Increase deprecated 'Use TStencilOp.Increase';
    soDecrease = TStencilOp.Decrease deprecated 'Use TStencilOp.Decrease';
    soInvert = TStencilOp.Invert deprecated 'Use TStencilOp.Invert';
  end;
  ...
  const
    sfNever = TStencilFunc.Never deprecated 'Use TStencilFunc.Never';
    sfLess = TStencilFunc.Less deprecated 'Use TStencilFunc.Less';
    sfLequal = TStencilFunc.Lequal deprecated 'Use TStencilFunc.Lequal';
    sfGreater = TStencilFunc.Greater deprecated 'Use TStencilFunc.Greater';
    sfGequal = TStencilFunc.Gequal deprecated 'Use TStencilFunc.Gequal';
    sfEqual = TStencilFunc.Equal deprecated 'Use TStencilFunc.Equal';
    sfNotEqual = TStencilFunc.NotEqual deprecated 'Use TStencilFunc.NotEqual';
    sfAlways = TStencilFunc.Always deprecated 'Use TStencilFunc.Always';
  end;
  ...
  TPrimitivesKindHelper = record helper for TPrimitivesKind
  const
    pkPoints = TPrimitivesKind.Points deprecated 'Use TPrimitivesKind.Points';
    pkLines = TPrimitivesKind.Lines deprecated 'Use TPrimitivesKind.Lines';
    pkTriangles = TPrimitivesKind.Triangles deprecated 'Use TPrimitivesKind.Triangles';
  end;
  ...
  public
    ...
    class function getContextID: Integer; virtual; abstract; // https://quality.embarcadero.com/browse/RSP-19160
    { Window handle }
...
begin
  ...
  FContextID := TContextManager.DefaultContextClass.getContextID; // https://quality.embarcadero.com/browse/RSP-19160
end;

destructor TTexture.Destroy;
begin
  if (FContextLostId <> 0) and (FContextResetId <> 0) then begin
    if TThread.CurrentThread.ThreadID <> MainThreadID then raise Exception.Create('TTexture created in the main thread must be also deleted in the main thread');
    TMessageManager.DefaultManager.Unsubscribe(TContextLostMessage, FContextLostId);
...
procedure TContextHelper.DrawCube(const Center, Size: TPoint3D; const Opacity: Single; const Color: TAlphaColor);
var
  I: Integer;
  A, B: TPoint3D;
  Ver: TVertexBuffer;
...
procedure TContextHelper.DrawRect(const TopLeft, BottomRight: TPoint3D; const Opacity: Single;
...
procedure TContextHelper.DrawLine(const StartPoint, EndPoint: TPoint3D; const Opacity: Single;
  const Color: TAlphaColor);
var
  Ver: TVertexBuffer;
  Idx: TIndexBuffer;
...
procedure TContextHelper.FillCube(const Center, Size: TPoint3D; const Opacity: Single; const Color: TAlphaColor);
var
  Ver: TVertexBuffer;
  Idx: TIndexBuffer;
  Mat: TColorMaterial;
...
procedure TContextHelper.FillPolygon(const Center, Size: TPoint3D; const Rect: TRectF;
...
begin
  ...
  begin
    ...
    if (TPlatformServices.Current <> nil) and TPlatformServices.Current.SupportsPlatformService(IFMXContextService,
      ContextSrv) then
...
begin
  AddEnumElementAliases(TypeInfo(TContextShaderArch), ['saUndefined', 'saDX9', 'saDX10', 'saGLSL', 'saMac', 'saIOS',
    'saAndroid']);
  AddEnumElementAliases(TypeInfo(TLightType), ['ltDirectional', 'ltPoint', 'ltSpot']);
  AddEnumElementAliases(TypeInfo(TProjection), ['pjCamera', 'pjScreen']);
  AddEnumElementAliases(TypeInfo(TMultisample), ['msNone', 'ms2Samples', 'ms4Samples']);
  AddEnumElementAliases(TypeInfo(TClearTarget), ['ctColor', 'ctDepth', 'ctStencil']);
  AddEnumElementAliases(TypeInfo(TStencilOp), ['soKeep', 'soZero', 'soReplace', 'soIncrease', 'soDecrease', 'soInvert']);
  AddEnumElementAliases(TypeInfo(TStencilFunc), ['sfNever', 'sfLess', 'sfLequal', 'sfGreater', 'sfGequal', 'sfEqual',
    'sfNotEqual', 'sfAlways']);
  AddEnumElementAliases(TypeInfo(TPrimitivesKind), ['pkPoints', 'pkLines', 'pkTriangles']);
end;

procedure UnregisterAliases;
begin
  RemoveEnumElementAliases(TypeInfo(TContextShaderArch));
  RemoveEnumElementAliases(TypeInfo(TLightType));
  RemoveEnumElementAliases(TypeInfo(TProjection));
  RemoveEnumElementAliases(TypeInfo(TMultisample));
  RemoveEnumElementAliases(TypeInfo(TClearTarget));
  RemoveEnumElementAliases(TypeInfo(TStencilOp));
  RemoveEnumElementAliases(TypeInfo(TStencilFunc));
  RemoveEnumElementAliases(TypeInfo(TPrimitivesKind));
end;

...
begin
  if info <> nil then
  begin
    AlphaColor := MakeColor(TGradient(info).InterpolateColor(inData^), GradientEvaluateAlpha);
    ...
    TBrushKind.Gradient:
      begin
        ...
        begin
          ...
          FShading := CGShadingCreateAxial(ColorSpace, StartPoint, StopPoint, FFunc, 1, 1);
        end
        else
        begin
          ...
          FShading := CGShadingCreateRadial(ColorSpace, CGPointMake(RCenter.X, RCenter.Y), ARect.Width / 2, CGPointMake(RCenter.X, RCenter.Y), 0, FFunc,
            1, 1);
        end;
      end;
    TBrushKind.Bitmap:
...
begin
  ...
  try
    ...
    Result := CGContextPathContainsPoint(ContextRef, CGPointMake(APoint.X, APoint.Y), kCGPathFillStroke) > 0;
  finally
        ...
        if WordWrap then
          ...
          case Trimming of
            ...
            TTextTrimming.Character:
              Wrapping :=  kCTLineBreakByCharWrapping; // Wrapping occurs before the first character that doesn't fit.
            TTextTrimming.Word:
              Wrapping := kCTLineBreakByWordWrapping; // Wrapping occurs at word boundaries unless the word itself doesn't fit on a single line
          end;

...
type
  ...
  private
    ...
    FBatchVertexBuffer: TBatchVertexBuffer; // Is not used in Update. Replaced by ExtBatchVertexBuffer
    FBatchIndexBuffer: TBatchIndexBuffer; // Is not used in Update. Replaced by ExtBatchIndexBuffer
...
// Temporary solution only for Update.
const
  ExtMaxBatchedVertices = 24576;
  ExtMaxBatchedIndices = 36864;

type
  TExtBatchVertexBuffer = packed array [0 .. ExtMaxBatchedVertices - 1] of TCanvasHelper.TVertexBufferItem;
  TExtBatchIndexBuffer = packed array [0 .. ExtMaxBatchedIndices - 1] of TCanvasHelper.TIndexBufferItem;

  TCanvasBatchBuffers = class
  public
    VertexBuffer: TExtBatchVertexBuffer;
    IndexBuffer: TExtBatchIndexBuffer;
  end;

  TBatchBuffers = class
  private
    FBuffers: TDictionary<Pointer, TCanvasBatchBuffers>;
  public
    constructor Create;
    destructor Destroy; override;

    procedure AllocateForCanvas(const ACanvasHelper: TCanvasHelper);
    procedure ReleaseForCanvas(const ACanvasHelper: TCanvasHelper);

    function GetBuffer(const ACanvasHelper: TCanvasHelper): TCanvasBatchBuffers;
  end;

var
  BatchBuffers: TBatchBuffers;

constructor TCanvasHelper.Create;
begin
  inherited;

  BatchBuffers.AllocateForCanvas(Self);
...
begin
  if BatchBuffers <> nil then
    BatchBuffers.ReleaseForCanvas(Self);
  FGradientMat.Free;
  FTexMat.Free;
  FSolidMat.Free;
...
var
  ...
  M: TMaterial;
begin
  if (MinVertexCount > ExtMaxBatchedVertices) or (MinIndexCount > ExtMaxBatchedIndices) then
  ...
  BatchInvalid := (FBatchedVertices + MinVertexCount >= ExtMaxBatchedVertices) or
    (FBatchedIndices + MinIndexCount >= ExtMaxBatchedIndices) or
...
var
  Buffer: TCanvasBatchBuffers;
begin
  Buffer := BatchBuffers.GetBuffer(Self);

  Buffer.VertexBuffer[FBatchedVertices].Position.X := Position.X;
  Buffer.VertexBuffer[FBatchedVertices].Position.Y := Position.Y;
  ...
  if (FCurrentTexture <> nil) and (FContext <> nil) and (TTextureStyle.RenderTarget in FCurrentTexture.Style) and
    ...
    Buffer.VertexBuffer[FBatchedVertices].TexCoord := TPointF.Create(TexCoord.X, 1 - TexCoord.Y)
  else
    Buffer.VertexBuffer[FBatchedVertices].TexCoord := TexCoord;
  if FContext <> nil then
    AlphaColorToPixel(Color, @Buffer.VertexBuffer[FBatchedVertices].VtxColor, FContext.PixelFormat)
  else
    Buffer.VertexBuffer[FBatchedVertices].VtxColor := Color;
...
begin
  BatchBuffers.GetBuffer(Self).IndexBuffer[FBatchedIndices] := Value;
  Inc(FBatchedIndices);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (CurVertices.Count >= ExtMaxBatchedVertices - 2) or (CurIndices.Count >= ExtMaxBatchedIndices - 2) then
        FlushBuffer;
    end;
...
var
  Index: Integer;
begin
  if (VertexCount < ExtMaxBatchedVertices) and (PrimitiveCount * 3 < ExtMaxBatchedIndices) then
  begin
...
procedure TBatchBuffers.AllocateForCanvas(const ACanvasHelper: TCanvasHelper);
begin
  TMonitor.Enter(Self);
  try
    FBuffers.Add(ACanvasHelper, TCanvasBatchBuffers.Create);
  finally
    TMonitor.Exit(Self);
  end;
end;

constructor TBatchBuffers.Create;
begin
  FBuffers := TObjectDictionary<Pointer, TCanvasBatchBuffers>.Create([doOwnsValues]);
end;

destructor TBatchBuffers.Destroy;
begin
  FreeAndNil(FBuffers);
  inherited;
end;

function TBatchBuffers.GetBuffer(const ACanvasHelper: TCanvasHelper): TCanvasBatchBuffers;
begin
  TMonitor.Enter(Self);
  try
    Result := FBuffers[ACanvasHelper];
  finally
    TMonitor.Exit(Self);
  end;
end;

procedure TBatchBuffers.ReleaseForCanvas(const ACanvasHelper: TCanvasHelper);
begin
  TMonitor.Enter(Self);
  try
    FBuffers.Remove(ACanvasHelper);
  finally
    TMonitor.Exit(Self);
  end;
end;
...
initialization
  BatchBuffers := TBatchBuffers.Create;
  RegisterAliases;

finalization
  FreeAndNil(BatchBuffers);
  UnregisterAliases;
end.

...
uses
  ...
  FMX.Platform.Screen.Android, FMX.Platform.Metrics.Android, ALFmxInertialMovement;
...
var aBackgroundColor: TAlphaColor;
    i: integer;
...
begin
  ...
  begin
    ...
    try
      ...
      // https://quality.embarcadero.com/browse/RSP-20971
      if (fStatusBarHeight > 0) and (Application.MainForm <> nil) then begin
        if not fcontext.CurrentStates[TContextState.csScissorOn] then glEnable(GL_SCISSOR_TEST);
        glScissor(0, FContext.Height, FContext.Width, round(fStatusBarHeight * Scale));
        aBackgroundColor := Application.MainForm.SystemStatusBar.BackgroundColor;
        glClearColor(TAlphaColorRec(aBackgroundColor).R / $FF, TAlphaColorRec(aBackgroundColor).G / $FF, TAlphaColorRec(aBackgroundColor).B / $FF, TAlphaColorRec(aBackgroundColor).A / $FF);
        glClear(GL_COLOR_BUFFER_BIT);
        if not fcontext.CurrentStates[TContextState.csScissorOn] then glDisable(GL_SCISSOR_TEST)
        else FContext.SetScissorRect(FContext.CurrentScissorRect);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DecorView.getWindowVisibleDisplayFrame(ContentRectVisible);  // Retrieve the overall visible display size in which the window this view is attached to
                                                                   // has been positioned in. This takes into account screen decorations above the window, for
                                                                   // both cases where the window itself is being position inside of them or the window is being
                                                                   // placed under then and covered insets are used for the window to position its content inside.
                                                                   // In effect, this tells you the available area where content can be placed and remain visible to users.
                                                                   // This function requires an IPC back to the window manager to retrieve the requested information, so should
                                                                   // not be used in performance critical code like drawing.
                                                                   //
                                                                   // DecorView.getDrawingRect return (0, 0, 640, 360)
                                                                   // DecorView.getWindowVisibleDisplayFrame return (25, 0, 640, 360)

      FContentRect.Top := Round(ContentRectVisible.Top / FScale);
      FContentRect.left := Round(ContentRect.left / FScale);
      FContentRect.Bottom := Round(ContentRect.bottom / FScale);
      FContentRect.Right := Round(ContentRect.right / FScale);
    end;
    FStatusBarHeight := FContentRect.Top;
    UpdateFormSizes;

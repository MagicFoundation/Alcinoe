...
Type
  ...
  TFilterColorAdjust = class(TALFilterBaseFilter)
  private
  ...
  published
    ...
    property Highlights: Single read GetHighlights write SetHighlights;
    // The Shadows offset.
      ...
      TEncoding.UTF8.GetBytes(
        ...
        *)
        ...
        'void main() {'+
          ...
          'vec4 color = texture2D(_Input, TEX0.xy);'+
          'vec4 result = color;'+

          'result = doTemperature(result);'+
          //'result = doExposure(result);'+               // BradLarson
          //'result = doGamma(result);'+                  // BradLarson
          'result = doExposureGamma(result);'+            // bestminr
          'result = doContrast(result);' +
          //'result = doHighlights(result);'+             // bestminr
          'result = doHighlightsAndShadows(result);'+     // BradLarson
          'result = doWhitesBlacks(result);'+
          'result = doSaturationVibrance(result);'+
...
begin
  Result := TFilterRec.Create('ALColorAdjust', 'Color adjustments operations.', [
    TFilterValueRec.Create('Contrast',    'The contrast multiplier.', 0,  -2,  2),
    TFilterValueRec.Create('Highlights',  'The highlights offset.',   1,   0,  1),  // 0,   0,  1 (bestminr) | 1,   0,  1 (BradLarson)
    TFilterValueRec.Create('Shadows',     'The shadows offset.',      0,   0,  1),
    TFilterValueRec.Create('Whites',      'The whites offset.',       0,  -1,  1),
    TFilterValueRec.Create('Blacks',      'The blacks offset.',       0,  -1,  1),
    TFilterValueRec.Create('Exposure',    'The exposure offset.',     0,  -1,  1),
    TFilterValueRec.Create('Gamma',       'The gamma offset.',        0,  -1,  1),
    TFilterValueRec.Create('Saturation',  'The saturation offset.',   0,  -1,  1),
    TFilterValueRec.Create('Vibrance',    'The vibrance offset.',     0,  -1,  1),
    TFilterValueRec.Create('Temperature', 'The temperature offset.',  0,  -1,  1),
    TFilterValueRec.Create('Tint',        'The tint offset.',         0,  -1,  1)]
  );
end;
...
{**********************************************}
function TFilterColorAdjust.GetContrast: Single;
begin
  if Filter <> nil then
...
{************************************************}
function TFilterColorAdjust.GetHighlights: Single;
begin
  if Filter <> nil then
...
{*********************************************}
function TFilterColorAdjust.GetShadows: Single;
begin
  if Filter <> nil then
...
{************************************************}
function TFilterColorAdjust.GetSaturation: Single;
begin
  if Filter <> nil then
...
{**********************************************}
function TFilterColorAdjust.GetVibrance: Single;
begin
  if Filter <> nil then
...
{********************************************}
function TFilterColorAdjust.GetWhites: Single;
begin
  if Filter <> nil then
...
{********************************************}
function TFilterColorAdjust.GetBlacks: Single;
begin
  if Filter <> nil then
...
{*************************************************}
function TFilterColorAdjust.GetTemperature: Single;
begin
  if Filter <> nil then
...
{******************************************}
function TFilterColorAdjust.GetTint: Single;
begin
  if Filter <> nil then
...
{**********************************************}
function TFilterColorAdjust.GetExposure: Single;
begin
  if Filter <> nil then
...
{*******************************************}
function TFilterColorAdjust.GetGamma: Single;
begin
  if Filter <> nil then
...
{*******************************************************}
procedure TFilterColorAdjust.SetContrast(AValue: Single);
begin
  if Filter <> nil then
...
{*********************************************************}
procedure TFilterColorAdjust.SetHighlights(AValue: Single);
begin
  if Filter <> nil then
...
{******************************************************}
procedure TFilterColorAdjust.SetShadows(AValue: Single);
begin
  if Filter <> nil then
...
{*********************************************************}
procedure TFilterColorAdjust.SetSaturation(AValue: Single);
begin
  if Filter <> nil then
...
{*******************************************************}
procedure TFilterColorAdjust.SetVibrance(AValue: Single);
begin
  if Filter <> nil then
...
{*****************************************************}
procedure TFilterColorAdjust.SetWhites(AValue: Single);
begin
  if Filter <> nil then
...
{*****************************************************}
procedure TFilterColorAdjust.SetBlacks(AValue: Single);
begin
  if Filter <> nil then
...
{**********************************************************}
procedure TFilterColorAdjust.SetTemperature(AValue: Single);
begin
  if Filter <> nil then
...
{***************************************************}
procedure TFilterColorAdjust.SetTint(AValue: Single);
begin
  if Filter <> nil then
...
{*******************************************************}
procedure TFilterColorAdjust.SetExposure(AValue: Single);
begin
  if Filter <> nil then
...
{****************************************************}
procedure TFilterColorAdjust.SetGamma(AValue: Single);
begin
  if Filter <> nil then
...
{**********************************************************}
constructor TALColorAdjustEffect.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
end;

{**************************************}
destructor TALColorAdjustEffect.Destroy;
begin
  inherited Destroy;
end;
